    ...
    <div class="bg-gray-100 w-7/12 mt-10 rounded-lg py-10 shadow-xl">
        <div class="flex items-center justify-center pt-2 flex-col">
            ...
            <a href="{{ route('teacher.correct.uploadpage', ['id' => $studenthomework->id, 'user_id' => $studenthomework->user_id]) }}"
                class="border-b-2 border-dotted italic text-blue-500 mt-2"
                >Add Your Correction &rarr;</a>
        </div>
      <div class="flex items-center justify-center pt-5 flex-col">
        <h1 class="text-gray-800 font-semibold text-xl mt-2 mb-2">{{ $studenthomework->User->name }}</h1>
        <h1 class="text-gray-800 font-semibold text-xl mt-2 mb-2">{{ $studenthomework->name }}</h1>
        <iframe src="{{ asset('studentassets/' . $studenthomework->file_path) }}" class="w-48 mb-2 shadow-xl" width="300" height="300" alt=""></iframe>
      </div>
    </div>
</div>
@endforeach
